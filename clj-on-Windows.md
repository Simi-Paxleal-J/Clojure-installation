...
`Invoke-Expression (New-Object System.Net.WebClient).DownloadString('https://download.clojure.org/install/win-install-1.10.3.849.ps1')`

or shorter

`iwr -useb download.clojure.org/install/win-install-1.10.3.849.ps1 | iex`

Alternatively, download the latest version of the installer and run the downloaded copy:

* https://download.clojure.org/install/win-install-1.10.3.849.ps1

When you run the installer, you will be prompted with several possible install locations:

```
PS Y:\Downloads> .\win-install-1.10.3.849.ps1
...
```powershell.exe -noprofile -executionpolicy bypass -file .\win-install-1.10.3.849.ps1```
